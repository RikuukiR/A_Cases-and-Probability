...
この教材は場合の数と確率の基本概念から応用まで学習するための LaTeX 教材です.
リアルタイムで PDF の変更を確認できる自動コンパイル機能付きです.

## ファイル構成

このプロジェクトには 2 つのバージョンがあります：

- **`main.tex`**: 簡潔版（定義と定理のみ）
- **`main_detailed.tex`**: 詳細版（教科書形式, 章立て構成）
...
# 詳細版（main_detailed.tex）- 教科書形式
uplatex main_detailed.tex && uplatex main_detailed.tex && dvipdfmx main_detailed.dvi
```
...
\fitblank[組合せ]{5}     % 「組合せ」という答えが入る5em幅の空欄
