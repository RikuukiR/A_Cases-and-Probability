...
\newcommand{\fitblankbf}[2][\relax]{%
  ...
  \else
    % 答えが指定されている場合
    \ifshowanswer
      \underline{\makebox[#2em][c]{\textcolor{blue}{\textbf{#1}}}}%
    \else
      \underline{\hspace*{#2em}}%
    \fi
  \fi
...
% インライン用（テキスト・数式両用）
% 使い方: \answertext{答え}
% 空欄時: 答えの幅 + 両端に2em（2文字分）の余白（手書き用）
\newcommand{\answertext}[1]{%
  \ifshowanswer
    \textcolor{blue}{#1}%  解答を青色で表示（下線なし）
  \else
    \uline{\hspace{2em}\phantom{#1}\hspace{2em}}%  空欄時は答えの幅 + 両端2em（手書き用）+ 黒の下線
  \fi
}
...
\newcommand{\answermath}[1]{%
  ...
  \else
    \underline{\hspace{3em}\phantom{#1}\hspace{3em}}%  空欄時は答えの幅 + 両端3em（手書き用）+ 黒の下線
  \fi
}
