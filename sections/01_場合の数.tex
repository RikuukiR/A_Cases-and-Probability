...
% 証明
例えば, 5個から3個取る組合せの総数は ${}_5\mathrm{C}_{3}$ で表され, それは 10 (通り) であるが, これを順列の総数から考えてみよう.

5個から3個取る順列の総数 ${}_5\mathrm{P}_{3}$ は, 次のように考えることもできる.

\[5個から3個取る組合せを作り, ({}_5\mathrm{C}_{3} \quad \text{通り})\]
\[取り出した3個すべてを1列に並べる(3! \quad \text{通り})\]

よって, 積の法則により, 次の等式が成り立つ.

\[{}_5\mathrm{C}_{3} \times 3! = {}_5\mathrm{P}_{3}\]

したがって, ${}_5\mathrm{C}_{3}  = \dfrac{\,{}_5\mathrm{P}_{3}\,}{\,3!\,} = \dfrac{\,5 \cdot 4 \cdot 3\,}{\,3 \cdot 2 \cdot 1\,} = 10$ （通り）
\vspace{1em}
...
\[_{8}\mathrm{C}_{2} = _{8}\mathrm{C}_{6}\]
\vspace{1em}
...
\end{exercise}
    ...
    \begin{proof}\mbox{}\\
        \indent(1)
        \begin{align*}
            _{6}\mathrm{C}_{3} &= \dfrac{\,6 \cdot 5 \cdot 4\,}{\,3 \cdot 2 \cdot 1\,} \\
                                &= 20 \quad \text{(個)}
        \end{align*}
        \vspace{0.5em}
        \indent(2)
        \begin{align*}
            _{6}\mathrm{C}_{2} &= \dfrac{\,6 \cdot 5\,}{\,2 \cdot 1\,} \\
                                &= 15 \quad \text{(本)}
        \end{align*}
        \vspace{0.5em}
        \indent(3)
        \begin{align*}
            _{6}\mathrm{C}_{4} &= \dfrac{\,6 \cdot 5 \cdot 4 \cdot 3\,}{\,4 \cdot 3 \cdot 2 \cdot 1\,} \\
                                &= 9 \quad \text{(本)}
        \end{align*}
    \end{proof}
    \vspace{1em}
